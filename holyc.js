...
const stderr = (value) => value;
const stdin = () => undefined;
const stdout = (value) => value;

const holyc_web_run = async () =>
  stdout(await output(parser(await lexer(init_hc()))));

const holy_node_idle = async (stdin) =>
  (hc.symtab.idle = hc.modes.HolyNode = true) &&
  (await output(parser(await lexer(init_hc(stdin)))));

const holy_node_script = async (stdin) => {
  throw await output(parser(await lexer(init_hc(stdin))));
};

const holy_script = async (stdin) =>
  (document.getElementById("stdout/stderr").innerText = await output(
    parser(await lexer(init_hc(stdin)))
  ));
...
const token_cases = [
  ...
  {
    char: "'",
    render: (hc) => lex_simple_quote_string(hc),
  },
  {
    char: '"',
    render: (hc) => lex_string(hc),
  },
  {
    char: "+",
    render: (hc) =>
  ...
  {
    char: ";",
    render: (hc) => new Token(";", token_type.semi, hc.lexer.line),
  },
  {
    char: ".",
    render: (hc) => new Token(".", token_type.dot, hc.lexer.line),
  },
  {
    char: "{",
    render: (hc) => new Token("{", token_type.rbrace, hc.lexer.line),
  },
  {
    char: "}",
    render: (hc) => new Token("}", token_type.lbrace, hc.lexer.line),
  },
  {
    char: "(",
    render: (hc) => new Token("(", token_type.rparen, hc.lexer.line),
  },
  {
    char: ")",
    render: (hc) => new Token(")", token_type.lparen, hc.lexer.line),
  },
  {
    char: ",",
    render: (hc) => new Token(",", token_type.comma, hc.lexer.line),
  },
  {
    char: "!",
    render: (hc) => new Token("!", token_type.not, hc.lexer.line),
  },
  {
    char: "=",
    render: (hc) =>
...
const lex_include = async (hc) => {
  ...
  hc.lexer.index += 2;
  hc.files.stdin =
    (await fetch(lex_string(hc).id)
      .then((response) => response.text())
      .then((text) => text)) +
    hc.files.stdin.slice(hc.lexer.index, hc.files.stdin.length);
  hc.lexer.index = 0;
...
const lex_alpha = (hc) => {
  ...
  return new Token(
    id,
    token_keywords.find((e) => e.id === id)?.type || token_type.id,
    hc.lexer.line
  );
};
...
const lexer_lex = async (hc) => {
  while ((hc.lexer.char = hc.files.stdin[hc.lexer.index++]) && hc.lexer.char) {
    let token = "";
...
const init_hc = (inpStdin) => {
  stderr && stderr("");
  (stdin()?.value && (hc.files.stdin = stdin().value)) ||
    (hc.files.stdin = inpStdin);
...
const lexer = async (hc) => {
  ...
  while (1) {
    ...
    if (
      token.type === token_type.js &&
      hc.files.stdin[hc.lexer.index + 1] === "{"
    ) {
      hc.lexer.index++;
      ...
      do {
        ...
        hc?.files?.stdin.substring(hc.lexer.index, hc.lexer.index + 2) !== "};"
      );
...
const list_eat_type = (tokenList) => {
  is_dtype(tokenList, hc.parser.index)
    ? hc.parser.index++
    : parser_error(tokenList[hc.parser.index]);
};
...
const list_eat_logical = (tokenList) => {
  is_logicalop(tokenList, hc.parser.index)
    ? hc.parser.index++
    : parser_error(tokenList[hc.parser.index]);
};
...
const list_eat_math = (tokenList) => {
  is_mathop(tokenList, hc.parser.index)
    ? hc.parser.index++
    : parser_error(tokenList[hc.parser.index]);
};
...
const list_eat_compassing = (tokenList) => {
  is_assingop(tokenList, hc.parser.index)
    ? hc.parser.index++
    : parser_error(tokenList[hc.parser.index]);
};
...
const parser_parse_class = (tokenList) => {
  ...
  if (check_token(tokenList, hc.parser.index, token_type.semi)) {
    ...
    hc.symtab.types.findIndex((e) => e.id === tokenList[hc.parser.index].id) >
      -1 && lexer_error(tokenList[hc.parser.index]);
...
const parser_parse_exp = (tokenList, arg, prototypeIndex, inClass) => {
  ...
  if (
    hc.symtab.prototypes.find((e) => e.id === tokenList[hc.parser.index].id) &&
    !check_token(tokenList, hc.parser.index - 1, token_type.number) &&
    !check_token(tokenList, hc.parser.index - 1, token_type.id)
  ) {
    ...
    if (check_token(tokenList, hc.parser.index, token_type.id)) {
      let procedureArg;
      if (prototypeIndex) {
        procedureArg = hc.symtab.prototypes[prototypeIndex - 1].args.find(
          (e) => e.id === tokenList[hc.parser.index].id
        );
        if (!procedureArg) {
          procedureArg = hc.symtab.scoped[prototypeIndex - 1]?.find(
            (e) => e.id === tokenList[hc.parser.index].id
          );
        }
      } else {
        procedureArg = hc.symtab.prototypes.find(
          (e) => e.id === tokenList[hc.parser.index].id
        );
      }
      ...
      if (check_token(tokenList, hc.parser.index + 1, token_type.dot)) {
        ...
        while (tokenList[hc.parser.index].type === token_type.id) {
    ...
    is_mathop(tokenList, hc.parser.index - 1) ||
...
const parser_parse_str_args = (tokenList) => {
  ...
  if (
    ...
    if (check_token(tokenList, hc.parser.index + 1, token_type.dot)) {
      ...
      while (tokenList[hc.parser.index].type !== token_type.semi) {
        tokenList[hc.parser.index].type === token_type.id &&
          ids.push(tokenList[hc.parser.index].id);
        hc.parser.index++;
      }
  ...
  if (i < hc.symtab.prototypes[prototype].args.length - 1) {
    if (ast) {
      ...
      if (
        ...
        !notAssigArgs ||
        check_token(tokenList, hc.parser.index, token_type.rparen)
      ) {
        return ast;
      }
    }
...
const parser_parse_call = (tokenList) => {
  if (
    hc.symtab.prototypes.findIndex(
      (e) => e.id === tokenList[hc.parser.index].id
    ) < 0
  ) {
    check_symtab(tokenList, true);
  ...
  if (hc.symtab.prototypes[prototype].args.length) {
    const notAssigArgs = hc.symtab.prototypes[prototype].args.filter((e) => {
      return e.id === undefined ? true : false;
    }).length;
    ...
    if (
      check_token(tokenList, hc.parser.index, token_type.str) &&
      !notAssigArgs &&
      tokenList[hc.parser.index].id === "*"
    ) {
      ...
      if (hc.symtab.prototypes[prototype].args.length) {
        ast.right = parser_parse_call_args(
          tokenList,
          prototype,
          notAssigArgs,
          0
...
const parser_parse_class_assig = (tokenList, index) => {
  ...
  let ids = [];
  while (tokenList[vars].type !== token_type.assig && tokenList[++vars]) {
    tokenList[vars].type === token_type.id && ids.push(tokenList[vars].id);
  }

  let isGlobal = false;
  if (
    index &&
    hc.symtab.prototypes[prototypeIndex]?.args?.findIndex(
      (e) => e.id === tokenList[hc.parser.index].id
    ) < 0
  ) {
    parser_error(tokenList[hc.parser.index]);
    ...
    check_symtab(tokenList, true);
    index = hc.symtab.global.findIndex(
      (e) => e.id === tokenList[hc.parser.index].id
    );
    isGlobal = true;
  }
...
const parser_parse_id = (tokenList, prototypeIndex) => {
  if (
    !hc.symtab.types.find((e) => e.id === tokenList[hc.parser.index].id) &&
    check_token(tokenList, hc.parser.index, token_type.id)
  ) {
  ...
  if (
    (prototypeIndex &&
      hc.symtab.prototypes[prototypeIndex - 1]?.args?.find(
        (e) => e.id === tokenList[hc.parser.index].id
      )) ||
    (prototypeIndex &&
      hc.symtab.scoped[prototypeIndex - 1]?.find(
        (e) => e.id === tokenList[hc.parser.index].id
      ))
  ) {
    parser_error(tokenList[hc.parser.index]);
  ...
  if (check_token(tokenList, hc.parser.index, token_type.semi)) {
    if (prototypeIndex) {
      !hc.symtab.scoped[prototypeIndex - 1] &&
        (hc.symtab.scoped[prototypeIndex - 1] = []);
      hc.symtab.scoped[prototypeIndex - 1].push({ ...symtabNode, value: 0 });
    } else {
      hc.symtab.global.push({ ...symtabNode, value: 0 });
    ...
    if (check_token(tokenList, hc.parser.index, token_type.comma)) {
      ...
      ast.left.right = parser_parse_inline_vars(
        tokenList,
        null,
        prototypeIndex
      );
    ...
    const prototypeIndex =
      hc.symtab.prototypes.findIndex((e) => e.id === prototypeId) + 1;
...
const parser_parse_prepostfix = (tokenList, block, prototypeIndex) => {
  ...
  if (is_mathop(tokenList, hc.parser.index)) {
    ...
    if (prototypeIndex) {
      let token = hc.symtab.prototypes[prototypeIndex]?.args?.findIndex(
        (e) => e.id === tokenList[hc.parser.index]
      );
      if (token < 0) {
        token = hc.symtab.scoped[prototypeIndex].findIndex(
          (e) => e.id === tokenList[hc.parser.index]
        );
        if (token < 0) {
          parser_error(tokenList[hc.parser.index]);
...
const parser_parse_ifelse = (tokenList, prototypeIndex) => {
  ...
  if (
    ...
    if (check_token(tokenList, hc.parser.index, token_type.if)) {
      ...
      ast.left.left.left.left.right = parser_parse_block(
        tokenList,
        prototypeIndex
      );
...
const parser_parse_for = (tokenList, prototypeIndex) => {
  ...
  ast.left.left.left.left.left.left.left.next.token =
    tokenList[hc.parser.index];
  list_eat(tokenList, token_type.lparen);
  ...
  ast.left.left.left.left.left.left.right = parser_parse_block(
    tokenList,
    prototypeIndex
  );

  ast.left.left.left.left.left.left.next.next = new AstNode(token_type.lbrace);
  ast.left.left.left.left.left.left.next.next.token =
    tokenList[hc.parser.index];
  list_eat(tokenList, token_type.lbrace);
    ...
    switch (ast.type) {
      case token_type.div:
        if (ast.right.token.type === token_type.number) {
          ...
          value /= parseInt(
            hc.symtab.global[get_symtab(ast.right.token)].value
          );
        }
        break;
      case token_type.mul:
...
const output_out_logical_exp = (ast, inside, prototypeIndex) => {
  ...
  if (first.type === token_type.id) {
    ...
    if (
      (token = hc.symtab.prototypes[prototypeIndex]?.args?.find(
        (e) => e.id === first.id
      ))
    ) {
      value.number = parseInt(token.value);
    } else if (
      (token = hc.symtab.scoped[prototypeIndex]?.find((e) => e.id === first.id))
    ) {
      value.number = parseInt(token.value);
    } else {
  ...
  while (walk) {
    if (
      ...
      if (walk.right.token.type === token_type.id) {
        mathFirst = parseInt(
          hc.symtab.global[get_symtab(walk.right.token)].value
        );
      } else {
...
const output_out_exp = (
  ...
  if (check_ast_type(ast?.token.type, "data_type")) {
    ...
    if (
      (prototypeIndex + 1 &&
        hc.symtab.prototypes[prototypeIndex]?.args?.find(
          (e) => e.id === procedureToken.id
        )) ||
      hc.symtab.scoped[prototypeIndex]?.find((e) => e.id === procedureToken.id)
    ) {
      prototypeArgIndex = hc.symtab.prototypes[prototypeIndex]?.args.findIndex(
        (e) => e.id === procedureToken.id
      );
      if (prototypeArgIndex < 0) {
        prototypeArgIndex = hc.symtab.scoped[prototypeIndex].findIndex(
          (e) => e.id === procedureToken.id
        );
      }
    } else {
  ...
  if (
    prototypeIndex + 1 &&
    (procedureArg = hc.symtab.prototypes[prototypeIndex]?.args.find(
      (e) => e.id === procedureToken.id
    ))
  ) {
        ...
        if (!walk.right?.right) {
          ...
          if (!procedureReturn) {
            if (symTabI >= 0) {
              ...
              hc.symtab.prototypes[prototypeIndex].args[prototypeArgIndex]
                ?.id === procedureToken.id
            ) {
              if (
                !Array.isArray(
                  hc.symtab.prototypes[prototypeIndex].args[prototypeArgIndex]
                ...
                hc.symtab.prototypes[prototypeIndex].args[
                  prototypeArgIndex
                ].value = [];
              }
              hc.symtab.prototypes[prototypeIndex].args[
      ...
      case token_type.assingdiv:
        if (walk.right.token.type === token_type.number) {
          ...
          if (
            (prototypeIndex + 1 &&
              (procedureArg = hc.symtab.prototypes[prototypeIndex]?.args.find(
                (e) => e.id === walk.right.token.id
              ))) ||
            (procedureArg = hc.symtab.scoped[prototypeIndex]?.find(
              (e) => e.id === walk.right.token.id
            ))
          ) {
            if (walk.right.token.type === token_type.classExp) {
              value /= procedureArg.value.find(
                (e) => e.id === classExpIds.join(".")
              )?.value;
            } else {
              value /= parseInt(procedureArg.value);
            }
        ...
        if (!procedureReturn) {
          if (symTabI >= 0) {
            ...
            hc.symtab.prototypes[prototypeIndex].args[prototypeArgIndex]?.id ===
            procedureToken.id
          ) {
      ...
      case token_type.assig:
        ...
        if (walk.right.token.type === token_type.number) {
          ...
          if (
            ...
            if (walk.right.token.type === token_type.classExp) {
              value = parseInt(
                hc.symtab.global[get_symtab(walk.right.token)].value.find(
                  (e) => e.id === classExpIds.join(".")
                )?.value
              );
            } else {
...
const output_out_ifelse = (ast, expList, prototypeIndex) => {
  ...
  let outBlockVal = "";
  if (logical) {
    outBlockVal = output_out_block(
      ast.left.left.left.right,
      expList,
      prototypeIndex
    );
  } else if (elseBlock) {
...
const output_out_for = (ast, expList, prototypeIndex) => {
  ...
  if (!symTabI) {
    symTabI = hc.symtab.prototypes[prototypeIndex]?.args?.findIndex(
      (e) => e.id === ast.left.token.id
    );
    if (symTabI < 0) {
      symTabI = hc.symtab.scoped[prototypeIndex]?.findIndex(
        (e) => e.id === ast.left.token.id
      );
    }
  }
  ...
  switch (cond.type) {
    case token_type.less:
      for (let i = val; i < condVal; i += iterateValue) {
        blockVal = output_out_block(
          ast.left.left.left.left.left.left.right,
          expList,
          prototypeIndex
        );
        ...
        if (hc.symtab.global[symTabI]?.id === ast.left.token.id) {
          ...
          hc.symtab.prototypes[prototypeIndex]?.args[symTabI]?.id ===
          ast.left.token.id
        ) {
          hc.symtab.prototypes[prototypeIndex].args[symTabI].value =
            parseInt(hc.symtab.prototypes[prototypeIndex].args[symTabI].value) +
            iterateValue;
        } else {
          hc.symtab.scoped[prototypeIndex][symTabI].value =
            parseInt(hc.symtab.scoped[prototypeIndex][symTabI].value) +
            iterateValue;
        }
      }
      break;
...
const output_out_procedures = (ast, expList) => {
  ...
  while (hc.symtab.prototypes[prototypeIndex].args[i] && aux) {
    if (aux.token.type === token_type.id) {
      hc.symtab.prototypes[prototypeIndex].args[i].value =
        hc.symtab.global[get_symtab(aux.token)].value;
...
const printf = (ast, prototypeIndex) => {
  ...
  if (ast.token.id.includes("%")) {
    ...
    if (
      (prototypeIndex + 1 &&
        (procedureArg = hc.symtab.prototypes[prototypeIndex]?.args.find(
          (e) => e.id === ast.left.right.token.id
        ))) ||
      (procedureArg = hc.symtab.scoped[prototypeIndex]?.find(
        (e) => e.id === ast.left.right.token.id
      ))
      ...
      if (ast.left.right.token.type === token_type.number) {
        ...
        if (ast.left.right.token.type === token_type.classExp) {
          ...
          str = str.replace(
            /%d/g,
            hc.symtab.global[get_symtab(args)].value.find(
              (e) => e.id === classexp.join(".")
            )?.value
          );
        } else {
          str = str.replace(
            /%d/g,
            hc.symtab.global[get_symtab(ast.left.right.token)].value
          );
        }
      }
    }
...
module.exports = { holy_node_script };
